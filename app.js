...
const quotes = [
  ...
  'Choose three things and let the rest be quiet.',
  ...
  'Let today surprise you with what three small steps can do.',
...
function renderTasks() {
  ...
  elements.buttonHint.textContent = count < 3
    ...
    : daily.selected.length ? 'Tap to choose a fresh set' : `${count} possibilities in your collection`;
}
...
function renderDaily() {
  ...
  daily.selected.forEach((id) => {
    const task = tasks.find((item) => item.id === id);
    if (!task) return;
    const card = document.createElement('li');
    card.className = `selected-card${daily.completed.includes(id) ? ' is-complete' : ''}`;
    card.dataset.id = id;
    card.innerHTML = `
      <button class="reroll-task" type="button" ${tasks.length <= 3 ? 'disabled' : ''} aria-label="Replace task: ${escapeAttribute(task.name)}">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 7h3c5 0 5 10 10 10h5M18 14l3 3-3 3M3 17h3c2.1 0 3.3-1.8 4.5-3.9M18 4l3 3-3 3M14 7h7"/></svg>
      </button>
  ...
  elements.progressLabel.textContent = hasSelection
    ? (completeCount === 3 ? 'All done — beautiful work' : `${completeCount} of 3 complete`)
    : 'Ready when you are';
  elements.randomizeLabel.textContent = hasSelection ? 'Choose Three New Tasks' : 'Select Random Tasks for the Day';
}
...
function selectRandomTasks() {
  ...
  setTimeout(() => {
    ...
    if (tasks.length > 3 && sameSet(candidates.slice(0, 3), daily.selected)) {
      candidates = [...candidates.slice(1), candidates[0]];
    }
    daily = { date: todayKey(), selected: candidates.slice(0, 3), completed: [] };
    saveState();
    renderDaily();
    renderTasks();
    elements.randomizeButton.classList.remove('is-shuffling');
    elements.randomizeButton.disabled = false;
    showToast('Your three are ready. One at a time.');
    if (navigator.vibrate) navigator.vibrate([20, 40, 20]);
  }, 720);
}

function rerollTask(id) {
  if (!daily.selected.includes(id) || tasks.length <= 3) {
    showToast('Add another task before replacing this one.');
    return;
  }
...
function toggleComplete(id) {
  ...
  renderDaily();
  renderHistory();
  if (daily.completed.length === 3) {
    celebrate();
    showToast('Three for three. You made today count.');
  }
}
